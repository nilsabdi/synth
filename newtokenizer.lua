   ...
   __index = {
      ...
      match = function(self, ...)
         ...
         for _, elem in pairs {...} do
            if type(elem) == 'string'
            or (type(elem) == 'table' and elem.meta and elem.meta.__token__)
            or (type(elem) == 'table' and elem.meta and elem.meta.__tokentype__)
            then
               local tok = self:inext()

               if not tok then
                  self.curr = save return
               end
