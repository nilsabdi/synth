...
function process(tokens, template, flags)
   ...
   if type(template.__config__.ignore) == "string" then
      template.__config__.ignore =
         iter(template.__config__.ignore:trim():split(" ")):map(
         function(e)
            return T(e)
         end
      )
      ...
      if segment.__config__.type == "parse" then
         ...
         if type(segment.__config__.ignore) == "string" then
            segment.__config__.ignore =
               iter(segment.__config__.ignore:trim():split(" ")):map(
               function(e)
                  return T(e)
               end
            )
         ...
         if tokens:peek() then
            error("Unexpected token" .. errfmt(tokens[tokens:getlast()] or tokens[tokens:getlast() - 1]))
         end
      ...
      if pattern:check(T "word") then
         ...
         if #ast > 0 then
            for _, ast in ipairs(ast) do
               ...
               local child =
                  assert(ast[alias], "no such child '" .. alias .. "' in node '" .. dump(meta(ast) and meta(ast).type) .. "'" .. dump(meta(ast).parent[1]))
               ...
               if meta(child) and meta(child).type then
                  ...
                  for _, t in ipairs(meta(child).type) do
                     pattern = assert(pattern[t] or findOrderedKey(pattern, t), "rule '" .. typename .. "' has no output defined " .. errfmt(nalias))
                  end
            ...
            if not ast[alias] then
               ...
               error("no such child '" .. alias .. "' in node '" .. dump(meta(ast).type) .. "'" .. errfmt(nalias))
            end
            ...
            if type(parent) == "table" and #parent > 0 then
               ...
               if meta(parent) and meta(parent).type then
                  ...
                  assert(type(parent) == "string" or (type(parent) == "table" and parent.meta.__token__), "empty list node" .. errfmt(nalias) .. dump(parent))
...
function apply(tokens, segment, rule, pattern)
   if type(pattern) == "string" then
      ...
      for name, child in pairs(pattern) do
         ...
         local ret = apply(tokens, segment, {[#rule + 1] = type(name) == "string" and name or nil, unpack(rule)}, child)
         if ret then
            return ret
         end
      end
...
function getpattern(segment, rule)
   ...
   for _, child in ipairs(rule) do
      pattern = assert(pattern[child] or findOrderedKey(pattern, child), "no such rule in segment: " .. child)
   end
   return pattern
end
...
function getops(pattern)
   ...
   while pattern:check(T "symbol") do
      ...
      if operators[op.lexeme] then
         error("operator already specified" .. errfmt(operators[op.lexeme]) .. errfmt(op))
      end
      ...
      for a = 0, #group - 1 do
         for b = a + 1, #group do
            if operators[group[a]] and operators[group[b]] then
               error("only one of " .. dump(table.keys(operators)) .. " can be used at a time" .. errfmt(operators[group[a]]) .. errfmt(operators[group[b]]))
            end
         end
      end
   end
...
function main()
   ...
   if ARGS[1] and ARGS[2] then
      synth(fetch(ARGS[2]), require(ARGS[1]))
   else
      print("parser TEMPLATE SOURCE")
   end
end
