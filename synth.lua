...
R = {
   ...
   OutputElement = setmetatable({
      ...
      {Interpole = function ()
         local __reset__, node = tokens.curr, {}
         repeat
            node.lerp = tokens:match(R.Interpolation)
            if not node.lerp then break end
            
            return setmetatable(node, {type=("OutputElement.Interpole"):split(".")})
         until true
   ...
   Interpolation = setmetatable({
      function ()
         local __reset__, node = tokens.curr, {}
         repeat
            if not tokens:match("${") then break end
            
            node.lerp = tokens:match(R.Name)
            if not node.lerp then break end
            
            if not tokens:match("}") then break end
            
            return setmetatable(node, {type=("Interpolation"):split(".")})
         until true
...
O = {
   ...
   OutputElement = {
      ...
      Interpole = function(self, interline, dent)
         local out = {}
         local interline = interline or false
         local dent = dent or 0
         
         out[#out+1] = 'out[#out+1] = output(assert_node(self, "'
         out[#out+1] = output(assert_node(self, "lerp"), interline, dent)
         out[#out+1] = '"), interline, dent)'
   ...
   Interpolation = function(self, interline, dent)
      local out = {}
      local interline = interline or false
      local dent = dent or 0
